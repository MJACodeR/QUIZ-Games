...
class ProgramGUI:
    ...
    def __init__(self, master):
        ...
        try:
            with open('C:\\Users\\Abhishek chaurasiya\\PycharmProjects\\Quiz Games\\Q-A-Python-Game-GUI-master\\data.txt', 'r') as data_file:
        ...
        self.button = Button(frame, text="Submit Answer",
    ...
    def loadQuestion(self):
        ...
        if(int(question['diff_level']) >= 4):
            ...
            self.diifficulty_level.config(
                text="This is a hard one - good luck!", fg="blue")
        else:
            self.diifficulty_level.grid_forget()

        self.question.config(text=self.current_question)
        self.question_status.config(text="%s/%s questions answered correctly" %
                                    (self.correct_question, self.no_of_question_answered))
    ...
    def checkAnswer(self):
        ...
        #when user ans is correct show on display
        ...
        #when user ans is INcorrect show on display
        else:
